...
class ExplicitDirectSolver(DirectSolver):
    ...
    def compute_rates(
        self, source: Source, field: SourceField, atmosphere: Atmosphere, bandpass: Bandpass
    ) -> u.Quantity:
...
class ExplicitScatteredSolver(ScatteredSolver):
    ...
    def compute_rates(
        ...
        rate_unit = (
            resolved.flx.unit * resolved.wvl.unit * resolved.weights.unit * scat_weights.unit
        )
...
class LUTDirectSolver(DirectSolver):
    ...
    def compile(
        ...
        **kwargs,
        ...
        self._luts[source] = UnitRegularGridInterpolator(
            (Z_range, *spectral_grid.points), res, method="linear", bounds_error=False
        )
        return 0
    ...
    def compute_rates(
        ...
        try:
            ...
            raise RuntimeError(
                f"No compiled LUT for {type(source).__name__}. Call compile() before predict()."
            ) from None
...
class LUTScatteredSolver(ScatteredSolver):
    ...
    def compute_rates(
        ...
        coord_array = np.broadcast_arrays(
            ...
            np.pi
            - np.abs(np.abs(eval_coords.az.rad[..., np.newaxis] - field.coords.az.rad) - np.pi),
        )
        lut_input = np.concatenate(
            [
                np.stack(coord_array, axis=-1),
                np.broadcast_to(field.spectral_data, eval_coords.shape + field.spectral_data.shape),
            ],
            axis=-1,
        )
