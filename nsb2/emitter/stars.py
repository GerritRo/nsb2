...
def from_gaia_dr3_catalog():
    ...
    return CatalogSource.from_photometric_catalog(
        coords, [G, gaia["phot_g_mean_mag"]], [[RP, BP], rp_bp], spec_lib, name="GaiaDR3_G<15"
    )


def from_gaia_dr3_map():
    gaia_down = download_file(
        "https://zenodo.org/records/15396676/files/gaia_mag15plus.npy", cache=True
    )
    ...
    return HEALPixSource.from_photometric_map(
        "icrs", [G, mag_map[0]], [[RP, BP], rp_bp], spec_lib, name="GaiaDR3_G>15"
    )


def from_gaia_suppl_catalog():
    xhip = np.genfromtxt(
        ASSETS_PATH / "anderson2012_xhip_suppl.dat", skip_header=3, delimiter=",", names=True
    )
    ...
    return CatalogSource.from_photometric_catalog(
        coords, [V, xhip["Vmag"]], [[V, B], v_b], spec_lib, name="XHIP_Gaia_Suppl"
    )
