...
class Kawara2017(Diffuse):
    """
    Diffuse galactic light from Kawara et. al (2017). The values are clipped to
    realistic values (max 35%) of starlight following Masana et. al (2021)
    """

    def compile(self):
        file = ASSETS_PATH + "IRIS_combined_SFD_really_nohole_nosource_4_2048.fits"
        self.hp_map = np.clip(hp.read_map(file, None) - 0.8, 0, 50)
        self.co_map = np.nan_to_num(
            np.load(ASSETS_PATH + "kawara_clipped.npy") / 0.35, nan=1
        )
    ...
    def SPF(self, lam, dust):
        ...
        return np.clip(
            dust[:, np.newaxis] * f1 - (dust**2)[:, np.newaxis] * f2, 0, None
        )
    ...
    def evaluate(self, frame, rays):
        ...
        weight = (
            1e-9 * self.SPF(frame.obswl.to(u.micron).value, dust) * corr[:, np.newaxis]
        )
        return rays * weight
