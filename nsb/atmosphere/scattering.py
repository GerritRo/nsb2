...
class Rayleigh(RadialScattering):
    """
    Implements Rayleigh Scattering following A. Bucholtz(1995) and the single scattering method from Kocifajc (2009)
    """
    ...
    def gradation(self, tau, scale, z, Z):
        ...
        return (
            sec_Z
            * scale
            * (np.exp(-sec_Z * tau) - np.exp(-sec_z * tau))
            / (sec_z - sec_Z)
        )
...
class Mie(RadialScattering):
    """
    Implements Mie Scattering following Henyey&Greenstein (1941) and the single scattering method from Kocifajc (2009)
    """
