...
class BlackBody():
    ...
    def _bpxvg(self, bandpass):
        if self.stis008 == False:
            vega = lambda x: np.pi/sigma*F_vega/T_vega**4*blackbody(x*1e-9, T_vega)
        else:
            vega = Vega_STIS008()
        x = np.linspace(bandpass.min, bandpass.max, 2**10+1)
        z = x*bandpass(x)*vega(x)

        return si.romb(z, dx=x[1]-x[0])
...
class SynthSpec():
    ...
    def __add__(self, synthspec):
        '''
        Stub, since implementing this is relatively difficult
        '''
